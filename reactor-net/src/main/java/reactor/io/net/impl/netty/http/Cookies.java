...
public class Cookies {
	...
	Map<String, Set<reactor.io.net.http.model.Cookie>> cachedCookies;
	...
	public Map<String, Set<reactor.io.net.http.model.Cookie>> getCachedCookies() {
		...
		Map<String, Set<reactor.io.net.http.model.Cookie>> cookies = new HashMap<>();
		for (String aCookieHeader : allCookieHeaders) {
			Set<Cookie> decode;
			if (isClientChannel) {
				final Cookie c = ClientCookieDecoder.STRICT.decode(aCookieHeader);
				Set<reactor.io.net.http.model.Cookie> existingCookiesOfName = cookies.get(c.name());
				...
				for (Cookie cookie : decode) {
					Set<reactor.io.net.http.model.Cookie> existingCookiesOfName = cookies.get(cookie.name());
					if (null == existingCookiesOfName) {
						existingCookiesOfName = new HashSet<>();
	...
	public static Cookies newServerRequestHolder(HttpHeaders headers) {
		return new Cookies(headers, reactor.io.net.http.model.HttpHeaders.COOKIE, false);
	}

	static final class NettyWriteCookie implements Cookie {

		final reactor.io.net.http.model.Cookie c;

		public NettyWriteCookie(reactor.io.net.http.model.Cookie c) {
			this.c = c;
		}
	...
	static final class NettyReadCookie extends reactor.io.net.http.model.Cookie {
