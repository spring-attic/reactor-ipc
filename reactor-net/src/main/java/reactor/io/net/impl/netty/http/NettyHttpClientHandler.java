...
public class NettyHttpClientHandler extends NettyChannelHandlerBridge {
    ...
    protected       Subscriber<? super HttpChannel<ByteBuf, ByteBuf>> replySubscriber;
    ...
    private void checkResponseCode(ChannelHandlerContext ctx, HttpResponse response) throws Exception {
        ...
        if (code == HttpResponseStatus.NOT_FOUND.code()
            ...
            if (replySubscriber != null) {
                Publishers.<HttpChannel<ByteBuf, ByteBuf>>error(ex).subscribe(replySubscriber);
            }
            discardBody = true;
        }
    ...
    public static final class ChannelInputSubscriberEvent {
        ...
        private final Subscriber<? super HttpChannel<ByteBuf, ByteBuf>> clientReplySubscriber;

        public ChannelInputSubscriberEvent(Subscriber<? super HttpChannel<ByteBuf, ByteBuf>> inputSubscriber) {
